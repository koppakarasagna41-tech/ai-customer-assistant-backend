...
def test_rag_list_documents(base_client: TestClient):
    ...
    base_client.post(
        ...
        json={"content": "Sample document listing test.", "filename": "list_test.txt"},
    )
...
def test_rag_search_endpoint(base_client: TestClient):
    ...
    payload = {"query": "vulnerability scanning", "top_k": 2}
...
def test_rag_query_endpoint(base_client: TestClient):
    # Full query pipeline
    payload = {"query": "What is security vulnerability scan?", "top_k": 3, "compress": True}
...
def test_rag_delete_document_success(base_client: TestClient):
    # Index one
    idx_res = base_client.post(
        "/api/v1/rag/index/text",
        json={"content": "To be deleted soon.", "filename": "delete_test.txt"},
    )
