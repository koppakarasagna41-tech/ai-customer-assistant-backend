...
@router.post(
    ...
    description=(
        "Manually trigger immediate generation of an enterprise "
        "performance report in the desired format (PDF, CSV, Excel, JSON)."
    ),
)
async def generate_custom_report(
    payload: ReportRequest, report_service: ReportService = Depends(get_report_service)
):
...
@router.get(
    ...
    description=(
        "Fetch a list of active and inactive daily, weekly, and monthly " "automated reports."
    ),
)
async def list_scheduled_reports(report_service: ReportService = Depends(get_report_service)):
...
@router.post(
    ...
    description=(
        "Establish a recurring schedule for reports, notifying "
        "designated email recipients automatically."
    ),
)
async def create_scheduled_report(
    payload: ScheduledReportConfig, report_service: ReportService = Depends(get_report_service)
):
    new_schedule = report_service.create_scheduled_report(payload)
    return BaseResponse(
        success=True, message="Automated report schedule created successfully.", data=new_schedule
    )
...
async def toggle_scheduled_report(
    ...
    if not updated_schedule:
        return BaseResponse(
            success=False, message=f"Schedule job with ID {schedule_id} not found.", data=None
        )
    return BaseResponse(
        success=True, message="Scheduled report status updated successfully.", data=updated_schedule
    )
