...
class SentimentResponse(BaseModel):
    ...
    escalation_recommended: bool = Field(
        ..., description="Whether emotional tone warrants agent escalation"
    )
...
@router.post(
    ...
    description=(
        "Analyzes sentiment of customer interaction and suggests if " "escalation is needed."
    ),
)
async def analyze_sentiment(payload: SentimentRequest):
    return BaseResponse(
        ...
        data=SentimentResponse(
            sentiment=SentimentDetail(label="negative", score=0.85), escalation_recommended=True
        ),
    )
