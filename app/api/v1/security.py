...
@router.post(
    ...
    description="Scan an incoming user prompt for injection, jailbreaks, hidden control character exploits, and generate detailed risk metadata."
)
async def validate_prompt_endpoint(
    ...
    return BaseResponse(
        success=True,
        message="Prompt security validation complete.",
        data=result
    )
...
@router.post(
    ...
    description="Check content against hate speech, harassment, self-harm, sexual content, violence, and malicious hacking patterns."
)
...
@router.get(
    ...
    description="Fetch real-time statistics regarding blocked queries, risk types, severity distributions, and a history of recent threat events."
)
async def get_security_analytics_endpoint(
    ...
    return BaseResponse(
        success=True,
        message="Security analytics summary compiled successfully.",
        data=analytics
    )
