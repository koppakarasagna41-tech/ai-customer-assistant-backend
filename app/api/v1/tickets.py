...
@router.post(
    ...
    description=(
        "Uses local heuristics to classify support tickets and suggest " "priority/category."
    ),
)
async def classify_ticket(payload: TicketCreate):
    ...
    if "billing" in desc_lower or "invoice" in desc_lower or "payment" in desc_lower:
        ...
        "sso" in desc_lower
        or "saml" in desc_lower
        or "login" in desc_lower
        or "oauth" in desc_lower
    ):
        category = "technical"
        priority = "urgent"
    ...
    return BaseResponse(
        ...
        data={"category": category, "confidence": 0.89, "priority": priority},
    )
...
async def create_ticket(
    payload: TicketCreate, service: TicketService = Depends(get_ticket_service)
):
    try:
        ticket = await service.create_ticket(payload)
        return BaseResponse(success=True, message="Ticket created successfully", data=ticket)
...
@router.get(
    ...
    description=(
        "Retrieves support tickets matching the specified filter criteria "
...
async def list_tickets(
    ...
    assigned_agent_id: str | None = Query(None, description="Filter by assigned agent"),
    search_query: str | None = Query(None, description="Search keyword matching title/description"),
    sort_by: str | None = Query("created_at", description="Field to sort by"),
    sort_order: SortOrder | None = Query(SortOrder.DESC, description="Sort order direction"),
    ...
    paginated = PaginatedData(items=items, total=total, page=page, size=size)
    return BaseResponse(success=True, message="Tickets retrieved successfully", data=paginated)
...
@router.get(
    ...
    description=(
        "Retrieves the overall, status-wise, priority-wise, and " "category-wise ticket statistics."
    ),
)
async def get_dashboard_stats(service: TicketService = Depends(get_ticket_service)):
    stats = await service.get_dashboard_stats()
    return BaseResponse(
        success=True, message="Dashboard statistics retrieved", data=TicketDashboardStats(**stats)
    )
...
@router.get(
    ...
    description=(
        "Retrieves a specific support ticket's details by ID including "
...
async def get_ticket(
    ...
    try:
        ...
        return BaseResponse(success=True, message="Ticket retrieved successfully", data=ticket)
    except ValueError as e:
...
async def update_ticket(
    ...
    try:
        ...
        return BaseResponse(success=True, message="Ticket updated successfully", data=ticket)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)) from e
...
async def delete_ticket(
    ...
    try:
        ...
        return BaseResponse(success=True, message="Ticket deleted successfully", data=success)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e)) from e
...
@router.post(
    ...
    description=(
        "Updates the status (open, in_progress, closed, escalated) of a "
        "ticket and logs a timeline event."
    ),
)
async def update_status(
    ...
    try:
        ...
        return BaseResponse(success=True, message="Ticket status updated", data=ticket)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)) from e
...
@router.post(
    ...
    description=(
        "Updates the priority (low, medium, high, urgent) of a ticket " "and logs a timeline event."
    ),
)
async def update_priority(
    ...
    try:
        ...
        return BaseResponse(success=True, message="Ticket priority updated", data=ticket)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)) from e
...
async def assign_agent(
    ...
    try:
        ...
        return BaseResponse(success=True, message="Agent assigned successfully", data=ticket)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)) from e
...
@router.post(
    ...
    description=(
        "Appends a new internal or public comment from an agent or "
        "customer to the ticket's thread."
    ),
)
async def add_comment(
    ...
    try:
        ...
        return BaseResponse(success=True, message="Comment added successfully", data=comment)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e)) from e
