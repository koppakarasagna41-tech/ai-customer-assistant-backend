...
class ChatService:
    ...
    async def process_chat_message(
        ...
        Stage 2: Gemini Execution, Output Validation, JSON parsing, Safety/Reflection checks, and Summary trigger.
        """
        ...
        input_tokens = TokenOptimizer.estimate_tokens(system_instruction) + sum(
            TokenOptimizer.estimate_tokens(part["text"])
            for c in contents for part in c["parts"]
        )
        ...
        response_schema = {
            ...
            "properties": {
                ...
                "entities": {"type": "OBJECT", "description": "Extracted key-value pairs (e.g. ticket_id)"},
                "suggested_actions": {
                    "type": "ARRAY",
                    "items": {"type": "STRING"},
            ...
            "required": ["response", "intent", "sentiment", "category", "urgency", "suggested_actions"]
        }
        ...
        try:
            ...
            logger.error(f"Malformed response format from Gemini client: {e}. Raw response: {raw_res}")
            raw_text = "{}"
        ...
        if TokenOptimizer.should_summarize(state.messages, threshold_tokens=2500):
            ...
            try:
                ...
                summary_prompt = self.prompt_builder.build_summary_prompt(state.summary, recent_msgs)
