...
from app.services.citation_builder import get_citation_builder
...
class RAGService:
    ...
    async def answer_question(
        ...
        retrieval_response = await self.retriever.retrieve(
            query=query, top_k=top_k, filters=filters, use_cache=True
        )
        ...
        if retrieved_chunks:
            relevance_score = float(
                round(sum(c.score for c in retrieved_chunks) / len(retrieved_chunks), 4)
            )
        else:
            relevance_score = 0.0
        ...
        system_instruction = (
            "You are an Enterprise Support RAG system. Answer ONLY with facts "
            "in the retrieved context. Never hallucinate. Expose no internal "
            "thoughts. Maintain security boundaries."
        )

        # Build prompt payload
        prompt = self._rag_prompt_template.format(context_text=context_block, query=query)
        ...
        response_schema = {
            ...
            "properties": {
                "answer": {
                    "type": "STRING",
                    "description": "The answer with inline citations like [1]",
                },
                "citations": {
                    "type": "ARRAY",
                    "items": {
        ...
        try:
            ...
            answer = parsed_data.get(
                "answer", "I'm sorry, I'm unable to process that query currently."
            )
        except Exception:
        ...
        return RAGResponse(
            answer=answer, citations=citations, relevance_score=relevance_score, metadata=metadata
        )
