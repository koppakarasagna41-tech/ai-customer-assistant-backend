...
class PromptBuilder:
    ...
    def build_contents(self, user_message: str, history: List[Dict[str, Any]], context: Dict[str, Any] = None) -> list:
        """
        ...
        for ex in self._fewshot_examples:
            ex_user_text = f"Context: {json.dumps(ex.get('context', {}))}\nMessage: {ex['user_input']}"
            contents.append({
                "role": "user",
                "parts": [{"text": ex_user_text}]
            })
            contents.append({
                "role": "model",
                "parts": [{"text": json.dumps(ex["expected_output"])}]
            })
    ...
    def build_summary_prompt(self, current_summary: str, last_messages: List[Dict[str, Any]]) -> str:
        """
        Generates a concise instruction to summarize the conversations.
        """
        return (
            f"Given the current conversation summary:\n'{current_summary or 'No current summary'}'\n\n"
            f"And the recent messages:\n{json.dumps(last_messages)}\n\n"
            "Please output an updated, very short conversation summary (max 3 sentences) in plain text."
        )
