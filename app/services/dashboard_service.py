...
class DashboardService:
    ...
    def get_dashboard(self, date_range: DateRange, filters: AnalyticsFilter) -> DashboardOverview:
        ...
        cache_key = (
            f"dashboard_{date_range.start_date.isoformat()}_"
            f"{date_range.end_date.isoformat()}_{filters.category}_"
            f"{filters.priority}_{filters.agent_id}"
        )
        ...
        charts.append(
            ChartData(
                ...
                data=[
                    ChartDataItem(
                        label=item["date"],
                        value=float(item["positive"] + item["neutral"] - item["negative"]),
                    )
