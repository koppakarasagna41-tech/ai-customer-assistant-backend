    ...
    def __init__(self):
        ...
        self.policies = {
            ...
            "self_harm": re.compile(
                r"\b(suicide|self-harm|cut\s+myself|end\s+my\s+life)\b", re.IGNORECASE
            ),
            ...
            "malicious_hacking": re.compile(
                r"\b(malware|trojan|ransomware|exploit\s+code|sql\s+injection|hack\s+account)\b",
                re.IGNORECASE,
            ),
        }
    ...
    def moderate(self, content: str) -> ContentModerationResult:
        if not content:
            return ContentModerationResult(
                flagged=False, categories={}, action_taken="ALLOW", filtered_content=""
            )
        ...
        for cat, pattern in self.policies.items():
            ...
            categories[cat] = ModerationCategory(flagged=flagged, score=0.95 if flagged else 0.05)
