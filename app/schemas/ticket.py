...
from app.models.ticket import TicketTimelineEvent, TicketComment
...
class TicketStatusUpdate(BaseModel):
    ...
    comment: Optional[str] = Field(None, description="Optional comment regarding the status change")

class TicketPriorityUpdate(BaseModel):
    priority: str = Field(..., description="New priority (low, medium, high, urgent)")
    comment: Optional[str] = Field(None, description="Optional comment explaining the priority change")

class TicketAgentAssign(BaseModel):
    agent_id: str = Field(..., description="ID of the agent being assigned")
    comment: Optional[str] = Field(None, description="Optional comment regarding the assignment")
...
class TicketDashboardStats(BaseModel):
    ...
    distribution_by_priority: Dict[str, int] = Field(..., description="Ticket distribution by priority")
    distribution_by_category: Dict[str, int] = Field(..., description="Ticket distribution by category")
