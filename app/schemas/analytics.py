...
class AnalyticsFilter(BaseModel):
    category: Optional[str] = Field(None, description="Optional filter by ticket category")
    priority: Optional[str] = Field(None, description="Optional filter by priority")
...
class RAGMetrics(BaseModel):
    ...
    avg_relevance_score: float = Field(..., description="Average semantic similarity relevance score")
...
class GeneralAnalytics(BaseModel):
    ...
    avg_response_time_min: float = Field(..., description="Average response time in minutes")
    customer_satisfaction_score: float = Field(..., description="Overall customer satisfaction index (0-5 or percentage)")
    ai_confidence_score: float = Field(..., description="Average confidence score from AI classifications and intents")
    intent_distribution: Dict[str, int] = Field(..., description="Count distribution of different ticket intents")
    sentiment_distribution: Dict[str, int] = Field(..., description="Count distribution of customer sentiments")
    ...
    agent_performance: List[AgentPerformanceDetail] = Field(..., description="Performance metrics breakdown per agent")
