...
class TicketRepository:
    ...
    def _seed_data(self):
        ...
        self._tickets["TCK-10001"] = Ticket(
            ...
            description="Every time I try to renew my subscription, I receive a Gateway Timeout error on checkout.",
            category="billing",
            priority="high",
            status="in_progress",
        ...
        self._tickets["TCK-10002"] = Ticket(
            ...
            description="Users from my team are redirected to a blank screen after successful authentication in Okta.",
            category="technical",
            priority="urgent",
            status="open",
            created_at=now,
        ...
        self._tickets["TCK-10003"] = Ticket(
            ...
            description="Could you provide the exact response and resolution target times for custom SLAs?",
            category="account",
            priority="low",
            status="closed",
    ...
    async def list_and_filter(
        ...
        async with self._lock:
            ...
            # Apply filters
            if filters.category:
                matched_tickets = [t for t in matched_tickets if t.category.lower() == filters.category.lower()]
            if filters.priority:
                matched_tickets = [t for t in matched_tickets if t.priority.lower() == filters.priority.lower()]
            if filters.status:
                matched_tickets = [t for t in matched_tickets if t.status.lower() == filters.status.lower()]
            if filters.assigned_agent_id:
                matched_tickets = [t for t in matched_tickets if t.assigned_agent_id == filters.assigned_agent_id]
            if filters.search_query:
                q = filters.search_query.lower()
                matched_tickets = [
                    t for t in matched_tickets 
                    if q in t.title.lower() or q in t.description.lower() or q in t.ticket_id.lower()
                ]
    ...
    async def get_stats(self) -> Dict[str, Any]:
        async with self._lock:
            ...
            for t in tickets:
                p = t.priority
                stats["distribution_by_priority"][p] = stats["distribution_by_priority"].get(p, 0) + 1
                c = t.category
                stats["distribution_by_category"][c] = stats["distribution_by_category"].get(c, 0) + 1
